...
ORDER BY
    total_mem DESC;
CREATE
OR REPLACE FUNCTION round5(ts timestamp) RETURNS timestamp AS $$ BEGIN RETURN date_trunc('hour', ts) + date_part('minute', ts):: int / 5 * interval '5 min';
END;
$$ LANGUAGE PLPGSQL;
...
SELECT
  ...
  AVG(
    (hi.total_mem - hu.memory_free)* 100 / total_mem
